...
%% Part I: Data Processing Tutorial
% This part of the tutorial will be conducted in lecture/discussion format
% with some examples to be worked through on paper and some on the computer. 
%
...
%%
% What are some easy methods to investigate the data? 

% e.g. matrix size, basic statistics, plots

whos
size(T) % size, max, min, length... 
mean(T) % basic statistics
plot (T,P)
axis ij
...
win=ones(5,1)/5; %window size equal to an array of ones with the number of data points you want to smooth over divided by the size of the array
...
plot (Tm*ones(size(P)), P,'k', (Tm+2*Tstd).*ones(size(P)), P, 'r:', (Tm-2*Tstd).*ones(size(P)), P, 'r:')
axis ij
legend ('T',  'meanT', 'meanT +/- 2*StdDev', ...
    'location', 'southwest') 
...
badinds=find(abs(T-Tm)>2*Tstd); %
%%
% Set all points with bad data to NaN; all other points stay the same.
%

Tdespike(badinds) = NaN;
Tdespike(1:4)=T(1:4); % reset the first few points to "good" - they are actually legitimately high!
...
Tdespike_sm=conv2(Tdespike, win, 'same');
...
plot (T, P,'y')
hold on
plot(Tdespike_sm, P, 'r', 'linewidth',2)
plot(actualT, actualP, 'g', 'linewidth',2)
axis ij
xlabel('temperature [^oC]')
ylabel('pressure [db]')
legend ('recorded T','smoothed T', 'actual T', 'location', 'southwest')
title ('Smoothing / Despiking tutorial')
...
%
% Look at your workspace. Jody has processed the Hallam lab data 
...
% possible before lab this week so you know what
% questions you have! 
%
